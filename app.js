...
function groupedRentalCardHTML(group) {
  ...
  const rows = group.map((r) => {
    ...
    const nombre = r.unitCount ? `${esc(r.machineName)} (unidad ${r.unitIndex} de ${r.unitCount})` : esc(r.machineName);
    return `
  ...
  return `
    <div class="modal-overlay" id="overlay">
      <div class="modal">
        ...
        <div class="row-line"><span class="lab">Máquina</span><span class="val">${esc(rental.machineName)} (${esc(rental.machineCode)})${rental.unitCount ? ` — unidad ${rental.unitIndex} de ${rental.unitCount}` : ""}</span></div>
...
async function bindRentalDetail() {
  ...
  document.getElementById("btn-toggle-paid")?.addEventListener("click", () => {
    ...
    state.rentals = state.rentals.map((r) => (r.id === rental.id ? { ...r, paid: nuevoValor, updatedAt: new Date().toISOString() } : r));
    saveRentals(state.rentals);
