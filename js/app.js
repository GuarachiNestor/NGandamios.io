...
function batchKeyOf(r) { return [r.groupId || r.id, r.machineId, r.dueDate, r.status, !!r.paid].join("|"); }
...
function viewGastos() {
  ...
  const totalGastosSemana = gastosSemana.reduce((s, g) => s + (Number(g.amount) || 0), 0);
  const totalGastosMes = gastosMes.reduce((s, g) => s + (Number(g.amount) || 0), 0);

  const gananciaSemana = weeklyIncome(weekStartToday) - totalGastosSemana;
  const gananciaMes = monthlyIncome(monthKeyToday) - totalGastosMes;

  let html = `
    <div class="section-title tag-font">Gastos</div>
    <div class="section-sub">Cargá tus gastos y mirá la ganancia neta (alquileres - gastos)</div>

    <div class="stat-grid">
      ${statCard("Gastos esta semana", fmtMoney(totalGastosSemana), "receipt", "var(--red)")}
      ${statCard("Ganancia esta semana", fmtMoney(gananciaSemana), "check", gananciaSemana >= 0 ? "var(--green)" : "var(--red)")}
      ${statCard("Gastos este mes", fmtMoney(totalGastosMes), "receipt", "var(--red)")}
      ${statCard("Ganancia este mes", fmtMoney(gananciaMes), "check", gananciaMes >= 0 ? "var(--green)" : "var(--red)")}
    </div>
...
function weeklyIncome(weekStart) {
  return visibleRentals()
      .filter((r) => getWeekStart(r.startDate || r.createdAt || todayISO()) === weekStart)
      .reduce((s, r) => s + (Number(r.total) || 0), 0);
}
function monthlyIncome(monthKey) {
  return visibleRentals()
      .filter((r) => (r.startDate || (r.createdAt || "").split("T")[0] || "").slice(0, 7) === monthKey)
      .reduce((s, r) => s + (Number(r.total) || 0), 0);
}
...
function bindGastos() {
  document.getElementById("btn-add-gasto")?.addEventListener("click", () => {
    ...
    const nuevo = { id: uid(), date, description, amount, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() };
  ...
  return `
    <div class="modal-overlay" id="overlay">
      <div class="modal">
        ...
        <div class="row-line"><span class="lab">Pago</span><span class="val">${rental.paid ? "Pagado" : "Pendiente de pago"}</span></div>
...
async function bindRentalDetail() {
  ...
  document.getElementById("btn-toggle-paid")?.addEventListener("click", () => {
    ...
    const nuevoValor = !rental.paid;
    state.rentals = state.rentals.map((r) => (batchIds.has(r.id) ? { ...r, paid: nuevoValor, updatedAt: new Date().toISOString() } : r));
    saveRentals(state.rentals);
    toast(nuevoValor ? "Marcado como pagado" : "Marcado como pendiente de pago");
    refreshRentalModal();
    renderContent();
  });
...
async function construirBlobReporteAlquileres() {
  ...
  rentalsReporte.forEach((r) => {
    ...
    if (!weekMap[weekStart]) weekMap[weekStart] = { count: 0, total: 0, gastos: 0 };
  ...
  rentalsReporte.forEach((r) => {
    ...
    if (!monthMap[mk]) monthMap[mk] = { count: 0, total: 0, gastos: 0 };
    monthMap[mk].count += 1;
  ...
  // --- Hoja 1: resumen semanal (con gastos y ganancia neta) ---
  ...
  weekKeys.forEach((wk) => {
    ...
    wsResumen.addRow({ semana: `${fmtDate(wk)} al ${fmtDate(addDays(wk, 6))}`, cant: info.count, monto: info.total, gastos: info.gastos, neta: info.total - info.gastos });
  });
  wsResumen.addRow({});
  wsResumen.addRow({ semana: "TOTAL GENERAL", cant: rentalsReporte.length, monto: totalGeneral, gastos: totalGastosGeneral, neta: totalGeneral - totalGastosGeneral });

  // --- Hoja 2: resumen mensual (con gastos y ganancia neta) ---
  ...
  monthKeys.forEach((mk) => {
    ...
    wsMensual.addRow({ mes: monthLabel(mk + "-01"), cant: info.count, monto: info.total, gastos: info.gastos, neta: info.total - info.gastos });
  });
  wsMensual.addRow({});
  wsMensual.addRow({ mes: "TOTAL GENERAL", cant: rentalsReporte.length, monto: totalGeneral, gastos: totalGastosGeneral, neta: totalGeneral - totalGastosGeneral });
  ...
  [...state.gastos].sort((a, b) => (a.date || "").localeCompare(b.date || "")).forEach((g) => {
    wsGastos.addRow({ fecha: fmtDate(g.date), desc: g.description || "", monto: g.amount || 0 });
  });
  wsGastos.addRow({});
  wsGastos.addRow({ fecha: "", desc: "TOTAL GASTOS", monto: totalGastosGeneral });
