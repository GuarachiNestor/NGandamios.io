...
async function collectBackupPayload() {
  ...
  return { version: 1, savedAt: new Date().toISOString(), machines: state.machines, rentals: state.rentals, deletedMachines: state.deletedMachines, photos };
}
...
async function construirBlobReporteAlquileres() {
  ...
  // --- Agrupar por semana (lunes a domingo) según fecha del alquiler (startDate) ---
  const weekMap = {};
  ...
  wsResumen.addRow({});
  wsResumen.addRow({ semana: "TOTAL GENERAL", cant: rentalsReporte.length, monto: totalGeneral });
